...
SimpleModel::SimpleModel(const char* basePath, const char* filename) noexcept
{
	...
	mVAORenderingInfos = DynArray<VAORenderingInfo>(shapes.size());
	mVAOBufferInfos = DynArray<VAOBufferInfo>(shapes.size());
...
{
	for (const VAORenderingInfo& info : mVAORenderingInfos) {
